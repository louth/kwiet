package au.com.louth.kwiet.actions.dialogflow
...
class DialogflowApp : AssistantApp<DialogflowRequest, DialogflowResponse> {
    ...
    /**
     * Returns the ChainableList constructed in Dialogflow response builder.
     *
     * @example
     * val app = DialogflowApp(request = req, response = res)
     *
     ...
     * app.handleRequest(actionMap)
     *
     * @return {ChainableList} ChainableList created in Dialogflow. If no ChainableList was created, an empty
     *     ChainableList is returned.
     * @dialogflow
     */
    fun getIncomingList(): ChainableList {
     ...
     *          .setTitle("Title of First ChainableList Item"),
     *        app.buildOptionItem(SELECTION_KEY_TWO,
     *          ["synonym of KEY_TWO 1", "synonym of KEY_TWO 2"])
     *          .setTitle("Title of Second ChainableList Item"),
     *      ]));
     * }
     *
    ...
    fun askWithList(richResponse: RichResponse, list: ChainableList): ResponseWrapper<DialogflowResponse>? {
        if (list.items.size < 2) {
            this.handleError("ChainableList requires at least 2 items")
            return null
        }
        return askWithResponseAndList(buildResponse(richResponse, true), list)
    }

    fun askWithList(inputPrompt: String?, list: ChainableList): ResponseWrapper<DialogflowResponse>? {
    ...
    private fun askWithResponseAndList(response: ResponseWrapper<DialogflowResponse>?, list: ChainableList): ResponseWrapper<DialogflowResponse>? {
        if (response == null) {
            error("Error in building response")
            return null
        }
