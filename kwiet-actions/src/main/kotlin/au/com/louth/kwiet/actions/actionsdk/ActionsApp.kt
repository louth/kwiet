...
import au.com.louth.kwiet.actions.*
import au.com.louth.kwiet.actions.ChainableList
...
//        }
    ...
    private fun buildAskHelper(inputPrompt: RichResponse?, possibleIntents: MutableList<ExpectedIntent>, dialogState: MutableMap<String, Any?>?): ResponseWrapper<ActionResponse>? {
        debug("ask: inputPrompt=$inputPrompt, dialogState=$dialogState")
     ...
     *     app.buildList("ChainableList title")
     *      .addItems([
     ...
     * @param {ChainableList} list ChainableList built with {@link AssistantApp#buildList|buildList}.
     * @param {Object=} dialogState JSON object the app uses to hold dialog state that
     *     will be circulated back by Assistant.
     * @return The response that is sent to Assistant to ask user to provide input.
     * @actionssdk
     */
    fun askWithList(inputPrompt: Any, list: ChainableList, dialogState: MutableMap<String, Any?>? = null): ResponseWrapper<ActionResponse>? {
        debug("askWithList: inputPrompt=$inputPrompt, list=$list, dialogState=$dialogState")
        if (list.items.size < 2) {
            handleError("ChainableList requires at least 2 items")
            return null
        }
...
//            state = json?.get("state") as String?
    ...
    override fun fulfillSystemIntent(intent: String, specType: String, intentSpec: NewSurfaceValueSpec, promptPlaceholder: String?, dialogState: MutableMap<String, Any?>?): ResponseWrapper<ActionResponse>? {
        TODO("not implemented for Actions SDK - please make an issue on actions-on-google-kotlin if this is needed")
    }

    override fun fulfillRegisterUpdateIntent(intent: String, specType: String, intentSpec: RegisterUpdateValueSpec, promptPlaceholder: String?, dialogState: MutableMap<String, Any?>?): ResponseWrapper<ActionResponse>? {
        TODO("not implemented for Actions SDK - please make an issue on actions-on-google-kotlin if this is needed")
    }
     ...
     * @actionssdk
     */
    private fun buildAskHelper(inputPrompt: InputPrompt?, possibleIntents: MutableList<ExpectedIntent>, dialogState: MutableMap<String, Any?>?): ResponseWrapper<ActionResponse>? {
...
//        val inputPrompt = buildInputPrompt(isSsml(simpleResponse.textToSpeech ?: ""), simpleResponse.textToSpeech ?: "")
    ...
    data class InputValueData(
            ...
            var listSelect: ChainableList? = null,
            ...
            var dialogSpec: DialogSpec? = null,
